...
MATTIE.preFabAPI.prefabMapId = MATTIE.static.prefabWorldId;
...
MATTIE.preFabAPI.loadPrefab = async function (event, args) {
	...
	preFabCopy.data.x=event.x
	preFabCopy.data.y=event.y
	preFabCopy._x=event.x
	preFabCopy._y=event.y
	preFabCopy.spawn(event.x, event.y);

	setTimeout(() => {
		MATTIE.visualEquip.objsQuedForDisposal[preFabCopy.data.id] = $gameMap.event(preFabCopy.data.id).mv3d_sprite;
	}, 500);
