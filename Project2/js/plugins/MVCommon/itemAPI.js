...
/**
 * give the player any item including items they already have
 */
MATTIE.itemAPI.anyItemInclusive = (list=$dataItems, cb="!item") => {
    let trys = 0;
    let allItems = list;
    let item;
    while((eval(cb) || item.name.length < 1) && ++trys < 20) item = allItems[MATTIE.util.randBetween(1,allItems.length-1)];

    if(trys < 20){
        $gameParty.gainItem(item,1);
        MATTIE.itemAPI.displayItemPickup(item);
    } else {
        MATTIE.itemAPI.displayItemPickup({"name":"literally nothing."});
    }
...
/**
 * @description get any item that the party does not have if they have one of every item, give them nothing
 * @param {*} list 
 * @returns 
 */
MATTIE.itemAPI.anyItemExclusive= (list=$dataItems) => {
    return MATTIE.itemAPI.anyItemInclusive(list, "(!item || $gameParty.hasItem(item) || item.name.length<1)")
}

/**
 * give player random weapon from list
 * @returns 
 */
MATTIE.itemAPI.anyWeaponInclusive= (list=$dataWeapons) => {
    return MATTIE.itemAPI.anyItemInclusive(list);
}

/**
 * give player random weapon they don't have from a list
 * @returns 
 */
MATTIE.itemAPI.anyWeaponExclusive= (list=$dataWeapons) => {
    return MATTIE.itemAPI.anyItemExclusive(list);
}

/**
 * give player random armor from list
 * @returns 
 */
MATTIE.itemAPI.anyArmorInclusive= (list=$dataArmors) => {
    return MATTIE.itemAPI.anyItemInclusive(list);
}

/**
 * give player random armor they don't have from a list
 * @returns 
 */
MATTIE.itemAPI.anyArmorExclusive= (list=$dataArmors) => {
    return anyItemExclusive(list);
}

MATTIE.itemAPI.displayItemPickup = (item) => {
    const clr = MATTIE.util.randBetween(1,31);
    MATTIE.msgAPI.displayMsg(`You pickup a \\c[${clr}]${item.name}`)
}