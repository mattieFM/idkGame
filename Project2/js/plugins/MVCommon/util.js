...
/**
 * @description get all comments from a event
 * @param {number} eventId the id of the event 
 * @returns 
 */
MATTIE.util.getAllEventComments = function (eventId) {
    // Get the event data using eventId
    ...
    // Retrieve the event's command list
    const commandList = event.list();
    let comments = [];
    
    // Loop through each command in the list
    commandList.forEach(command => {
        // Code 108 is the first line of a comment, 408 is continuation lines
...
/**
 * linerly interpret and then back between 2 numbers
 * @param {number} a number 1
 * @param {number} b number 2
 * @param {number} alpha val between 0-2 of which number we are at or greater...
 */
MATTIE.util.lerpLoop = function (a, b, alpha) {
	alpha= alpha % 2;
