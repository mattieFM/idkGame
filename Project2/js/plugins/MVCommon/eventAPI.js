...
MATTIE.eventAPI.updatePosOfRunTimeEvents = function () {
	...
	for (let index = 0; index < keys.length; index++) {
		...
		if (dataEvent) {
			if ((dataEvent.mapId === $gameMap.mapId() || dataEvent.allMaps) && dataEvent.persist) {
...
/**
 * @description get a list of all event ids that have a matching meta tag
 * @param {*} tag 
 */
MATTIE.eventAPI.findAllEventsWithMetaTagOnMap = function(tag){
	let ids = [];
	$dataMap.events.forEach(ev=>{
		if(ev){
			if(ev.meta[tag]){
				ids.push($gameMap.event(ev.id));
			}
		}
	})
	return ids;
